...
export async function createDoctor(input: CreateDoctorInput) {
  try {
    ...
    if(error?.code === "P2002") {
      throw new Error("A doctor with this email is already exist");
    }
...
export async function updateDoctor(input: UpdateDoctorInput) {
  try {
    ...
    // if email is changing check if the new email exist or not
    if(input.email !== currentDoctor.email) {
      ...
      if(existingDoctor) {
        throw new Error("There is already a doctor with this email")
      }
    }
